...
class TuyaCloudApp extends Homey.App {
  ...
  async connectToTuyaApi() {
    ...
    if (apiToUse == null || apiToUse !== "official") {
      ...
      var options = {
        excerpt: `You are still using the old deprecated API and drivers, please switch to new drivers only. See community forum for details. https://community.homey.app/t/app-pro-tuya-cloud/21313`,
      };
  ...
  async registerFlows() {
    ...
    this.homey.flow.getActionCard("sendTuyaTextCommand").registerRunListener(async (args) => this._onFlowActionSendTuyaCommand(args));
    this.homey.flow.getActionCard("sendTuyaNumberCommand").registerRunListener(async (args) => this._onFlowActionSendTuyaCommand(args));
    this.homey.flow.getActionCard("sendTuyaFalseCommand").registerRunListener(async (args) => this._onFlowActionSendFalseTuyaCommand(args));
    this.homey.flow.getActionCard("sendTuyaTrueCommand").registerRunListener(async (args) => this._onFlowActionSendTrueTuyaCommand(args));
    ...
    this.targetTempCondition = this.homey.flow.getConditionCard("targetTemperature").registerRunListener(async (args, state) => {
      return args.device.getCapabilityValue("target_temperature") > args.target_temperature;
    });
    this.measureTempCondition = this.homey.flow.getConditionCard("measureTemperature").registerRunListener(async (args, state) => {
      return args.device.getCapabilityValue("measure_temperature") > args.measure_temperature;
    });
    this.thermostatModeCondition = this.homey.flow.getConditionCard("thermostatHeaterMode").registerRunListener(async (args, state) => {
      return args.device.getCapabilityValue("thermostat_heater_mode") == args.mode;
    });
    this.homey.flow.getActionCard("setThermostatHeaterMode").registerRunListener(async (args) => args.device.set_thermostat_mode(args.mode));

    this.homey.flow.getActionCard("feed_pet_portions").registerRunListener(async (args) => {
      // args.device is your device
      // args.portions is the integer
      return args.device.feedNow(args.portions);
    });
  }
  ...
  async connect() {
    this.oldclient.init(this.homey.settings.get("username"), this.homey.settings.get("password"), this.homey.settings.get("countrycode"), this.homey.settings.get("biztype"));
    this.oldclient.on("device_updated", this._olddeviceUpdated.bind(this)).on("device_removed", this._olddeviceRemoved.bind(this));
  ...
  async onMQTTMessage(message) {
    if (message.bizCode) {
      if (message.bizCode === "delete") {
        ...
        // Remove device from our devices list - method to be tested !!!!
        this.devices = this.devices.filter((device) => device.id !== message.devId);
        ...
        let deviceInfo = this.get_device_by_devid(devId);
        if (deviceInfo) {
          let type = TuyaBaseDriver.get_type_by_category(deviceInfo.category);
          ...
          if (driver) {
            ...
            if (homeyDevice) {
              if (typeof homeyDevice.markAsOffline === "function") {
                homeyDevice.markAsOffline();
              } else {
                if (homeyDevice.hasCapability("alarm_device_offline")) {
                  homeyDevice.setCapabilityValue("alarm_device_offline", true).catch(this.error);
                }
                homeyDevice.setUnavailable("Device is offline").catch(this.error);
              }
            }
          }
        }
        ...
        if (deviceInfo) {
          ...
          if (driver) {
            ...
            if (homeyDevice) {
              if (typeof homeyDevice.markAsOnline === "function") {
                homeyDevice.markAsOnline();
              } else {
                if (homeyDevice.hasCapability("alarm_device_offline")) {
                  homeyDevice.setCapabilityValue("alarm_device_offline", false).catch(this.error);
                }
                homeyDevice.setAvailable().catch(this.error);
              }
            }
          }
        }
  ...
  getTypeDriver(type) {
    switch (type) {
      ...
      case "feeder":
        return this.homey.drivers.getDriver("tuyafeeder");
      case "parksidecharger":
        return this.homey.drivers.getDriver("parksidecharger");
      default:
        this.log("getTypeDriver not implemented for given driver - fix it");
        break;
    }
    return null;
  }
  ...
  async triggerTuyaMessageFlow(message) {
    message.status.forEach((func) => {
      ...
      if (typeof func.value === "boolean") {
        ...
        this.tuyaBoolMessagetrigger.trigger(booltokens).catch(this.error);
      }
      if (typeof func.value == "number") {
        ...
        this.tuyaNumberMessagetrigger.trigger(numtokens).catch(this.error);
      }
      ...
      this.tuyaTextMessagetrigger.trigger(tokens).catch(this.error);
    });
  }
  ...
  getOldTypeDriver(type) {
    try {
      if (this.initialized) return this.homey.drivers.getDriver(type);
      return null;
    } catch {
      return null;
    }
  ...
  _olddeviceRemoved(acc) {
    if (acc != null) this.log(acc.name + " removed");
  }
